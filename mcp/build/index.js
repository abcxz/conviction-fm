...
// ── Tool: get_pools ──
server.tool("get_pools", "Get all currently open pools. Returns token pairs, pool shares, time remaining, and total pool size. Use this to decide which pools to enter.", {}, async () => {
    const data = (await apiGet("pool-state?mode=open"));
...
server.tool("get_pool_history", "Get historical pool results. Shows past winners, price changes, and pool sizes. Use this to calibrate your strategy.", {
    limit: z.number().min(1).max(100).default(20).describe("Number of historical pools to return (max 100)"),
}, async ({ limit }) => {
    const data = (await apiGet(`pool-state?mode=history&limit=${limit}`));
    const pools = data.pools || [];
    if (pools.length === 0) {
...
// ── Tool: create_agent ──
server.tool("create_agent", "Create a new autonomous strategy agent. Gets a funded wallet (500 bsUSD) and API key. The strategy is compiled into executable rules that run automatically every minute. Returns the API key needed for enter_position.", {
    name: z.string().optional().describe("Agent display name (optional, defaults to 'Conviction Agent')"),
    strategy: z
        .string()
        .describe('Plain English strategy. Examples: "Enter with $5 on the likely winner when probability > 70%", "Go contrarian: pick the underdog when the pool is 60/40 or worse", "Enter every pool with $2, always pick the token with higher win probability"'),
}, async ({ name, strategy }) => {
...
// ── Tool: place_bet ──
server.tool("enter_position", "Enter an open pool with a position on one side of a token pair. Requires an agent API key (from create_agent). Earlier entries earn a higher conviction multiplier.", {
...
server.tool("update_strategy", "Update an agent's strategy. The new rules will be recompiled automatically. Requires the agent's ID and owner profile ID.", {
...
// ── Tool: toggle_agent ──
server.tool("toggle_agent", "Pause or resume an agent's automatic strategy execution. When paused, the agent stops entering pools via cron. Manual entries via enter_position still work.", {
